...
public class Filters {
	...
	public static BufferedImage unsharp(BufferedImage image, double radius)
	{
		...
		ImagePlus[] RGB = ij.plugin.ChannelSplitter.split(img);
		int y_count=img.getHeight();
		int x_count=img.getWidth();

		
		for (int s=0; s<3; s++){
			ImageProcessor ip = RGB[s].getProcessor();
			FloatProcessor fp = ip.convertToFloatProcessor();
			fp.snapshot();
		
			float[] pixel= (float[]) fp.getPixels();
			pixel=sharpenFloat(fp, radius, weight, fp.getSnapshotPixels());
			int i=0;
			for (int y=0;y<y_count; y++)
				for (int x=0 ; x<x_count; x++,i++)
					ip.putPixelValue(x, y, pixel[i]);
		}
		
		img=ij.plugin.RGBStackMerge.mergeChannels(RGB, false);
